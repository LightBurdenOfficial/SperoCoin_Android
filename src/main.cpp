...
const string strMessageMagic = "SperoCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // SperoCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // SperoCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
    ...
    if (!fTestNet && !tx.IsStandard() && !GetBoolArg("-acceptnonstdtxn", false))
    ...
    if (fCheckInputs)
    {
        ...
        if (!tx.AreInputsStandard(mapInputs) && !fTestNet && !GetBoolArg("-acceptnonstdtxn", false))
...
// SperoCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes SperoCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // SperoCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // SperoCoin: clean up wallet after disconnecting coinstake
    BOOST_FOREACH(CTransaction& tx, vtx)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // This rule applies to all blocks whose timestamp is after Agosto 1, 2016, 0:00 UTC.
    // On testnet it is enabled as of Agosto 1, 2016, 0:00 UTC.
    if (pindex->nTime > 1470009600 || (fTestNet && pindex->nTime > 1467331200))
        BOOST_FOREACH(CTransaction& tx, vtx)
        {
            CTxIndex txindexOld;
...
if(pindexBest->nHeight > POS_POW_HIBRID)
{
    ...
    if (IsProofOfStake())
    {
        // SperoCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
...
// SperoCoin: total coin age spent in transaction, in the unit of coin-days.
...
// SperoCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // SperoCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // SperoCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // SperoCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // SperoCoin: compute stake modifier
...
//  printf("centurionMiner block accepted!!\n");
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 5; //coinbase maturity change to 180 blocks
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // SperoCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // SperoCoin: check proof-of-stake
    ...
    // SperoCoin: ask for pending sync-checkpoint if any
    ...
    {
printf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), pblock->hashPrevBlock.ToString().substr(0,20).c_str());
  if (pfrom) {
      // SperoCoin: check proof-of-stake
      if (pblock->IsProofOfStake())
      {
      ...
      // Ask this guy to fill in what we're missing
          ...
          // SperoCoin: getblocks may not obtain the ancestor block rejected
...
// sperocoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SperoCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x6f5dd836da72471e5ea8f3c836249e447059f9f7c7c26a2f91fecd0c509dd677"));
        ...
        // SperoCoin: initialize synchronized checkpoint
...
// reset counts to detect forked peers
        pfrom->nHighestHeightRequested = 0;
        pfrom->nHeightBackwards = 0;
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // SperoCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        // detect when peers go backwards in the height of blocks that they request, which is a sign of a forked client stuck in an endless loop. if this happens too many times, ban the peer.
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // SperoCoin: tell downloading node about the latest block if it's
