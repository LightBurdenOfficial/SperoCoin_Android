...
{
    // Sperocoin: check prefix
    if(uri.scheme() != QString("sperocoin"))
        return false;
...
{
    // Convert SperoCoin:// to SperoCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sperocoin://"))
    {
        uri.replace(0, 12, "sperocoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SperoCoin.lnk";
}
...
{
    return GetAutostartDir() / "SperoCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SperoCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  SperoCoin-qt [" + tr("command-line options") + "]                     " + "\n";
